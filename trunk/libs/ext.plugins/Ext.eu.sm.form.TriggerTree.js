...
Ext.eu.sm.form.TriggerTree = Ext.extend(Ext.form.TriggerField, {
	defaultAutoCreate	: {
		tag					: "input",
		type				: "text",
		size				: "24",
	...
	initComponent : function(){
		...
		Ext.eu.sm.form.TriggerTree.superclass.initComponent.call(this);
	},


	onRender : function(ct, position){
		var that = this;
		Ext.eu.sm.form.TriggerTree.superclass.onRender.call(this, ct, position);
	...
	initValue : function(){
		...
		if(node = Ext.eu.sm.tree.utils.findDeepChildNode(that.treeRoot,'id',that.value)){
			that.select(node, true);
		}
		Ext.eu.sm.form.TriggerTree.superclass.initValue.call(this);
	...
	initEvents : function(){
		...
		Ext.eu.sm.form.TriggerTree.superclass.initEvents.call(this);
	...
	onDestroy : function(){
		...
		Ext.eu.sm.form.TriggerTree.superclass.onDestroy.call(this);
	},
	...
	onResize: function(w, h){
		var that = this;
		Ext.eu.sm.form.TriggerTree.superclass.onResize.apply(this, arguments);
	...
	onEnable: function(){
		var that = this;
		Ext.eu.sm.form.TriggerTree.superclass.onEnable.apply(this, arguments);
		if(that.hiddenField){
	...
	onDisable: function(){
		var that = this;
		Ext.eu.sm.form.TriggerTree.superclass.onDisable.apply(this, arguments);
		if(that.hiddenField){
	...
	onLoad : function(){
		...
		if(root.hasChildNodes()){
			...
			if(that.lastQuery == that.allQuery){
				...
				var n;
				if(n = Ext.eu.sm.tree.utils.findDeepChildNode(root,'text',that.value)){
					that.select(n, true);
				}
			}
		}else{
	...
	getValue : function(){
		...
		if(that.valueField){
			...
			return Ext.eu.sm.form.TriggerTree.superclass.getValue.call(this);
		}
	},
	...
	setValue : function(v,textv){
		...
		if(!textv && that.treeRoot){
			var node = Ext.eu.sm.tree.utils.findDeepChildNode(that.treeRoot,that.valueField,v);
			if(node){
				if(that.displayPath){
					textv = '';
				...
				if(!that.doSelectEvent || that.fireEvent('beforeselect', that, v,that.value,node) !== false ){
					...
					Ext.eu.sm.form.TriggerTree.superclass.setValue.call(this, textv);
					that.value = v;
...
Ext.reg('triggertree', Ext.eu.sm.form.TriggerTree);