//https://raw.githubusercontent.com/kir/js_cursor_position/master/src/cursor_position.js
if (!window.maxkir)
	maxkir = {};
...
maxkir.CursorPosition = function(element, padding) {
	...
	this.split_to_lines = function() {
		...
		return splitter.splitString(element.value.substr(0, pos),
				innerAreaWidth);
	};
...
/**
 * Return preferred [width, height] of the text as if it was written inside
 * styledElement (textarea)
 *
 * @param styledElement
 *            element to copy styles from
 ...
 * Usage:
 *
 * var range = new maxkir.SelectionRange(textarea); var selection =
 * range.get_selection_range() var selectionStart = selection[0] var
 * selectionEnd = selection[1] On a error, returns [0,0]
 *
...
maxkir.StringSplitter.prototype.splitString = function(s, max_width) {
	...
	var cut_off = function(idx) {
		...
		if (remaining.length > 0) {
			return [ s.substr(0, idx + 1) ].concat(that.splitString(remaining,
					max_width));
		}
