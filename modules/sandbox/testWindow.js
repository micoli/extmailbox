Ext.app.Modules.testWindow = Ext.extend(Ext.app.Module, {
	id		:'test2-win',
	init	: function(){
		var that = this;
		Ext.ns('Ext.eu.sm');
		...
		Ext.Ajax.request({
			url		: 'proxy.php',
			params	: {
				exw_action	: 'local.batchAffilies.getPPActivities'
			},
			success	: function(data){
				var json = JSON.parse(data.responseText);
				Ext.eu.sm.PPActivities={}
				Ext.each(json.results,function(activity){
					Ext.eu.sm.PPActivities['_'+activity.category		]=activity.libelle_category		;
					Ext.eu.sm.PPActivities['_'+activity.main_category	]=activity.main_libelle_category;
				})
			},
	...
	createWindow : function(){
		...
		if(!win){
			...
			/*var winDynamicForm = new Ext.Window({
				modalContainer		: win,
				title				: 'test dynamicForm',
				maximizable			: true,
				resizable			: true,
				width				: 700,
				height				: 400,
				...
				items				: [{
					...
					id					: dynFormId,
					frame				: true,
					dataSource			: '{"f1":123,"f2":222,"f3":333,"f5":[{"c1":"c1","c2":"c2","c3":"aaa"},{"c1":"ac1","c2":"ac2","c3":"ccc"}]}',
					dynamicConfig		: [{
						type	: 'text',
						label	: '-f1',
						name	: 'f1'
					...
					items	: [{
						...
						tbar	: [{
							...
							handler : function(){
								if (typeof document.winYoutube1 == 'undefined'){
									document.winYoutube1 = new Ext.Window({
										modalContainer		: win,
							...
							handler : function(){
								if (typeof document.winYoutube2 == 'undefined'){
									document.winYoutube2 = new Ext.Window({
										modalContainer		: win,
										title				: 'test eu.sm.youtube',
										maximizable			: true,
										resizable			: true,
						...
						items		:[{
							...
							pivot		: {
								...
								columnModel		: {
									header: 'aaaaa'	, width: 60, sortable: true, fixed:false,dataIndex: 'value',
							...
							title	: "eee",
							xtype	: 'Ext.eu.sm.tagManagerConfig'
						}]
					},{
						xtype	: 'panel',
						region	: 'east',
